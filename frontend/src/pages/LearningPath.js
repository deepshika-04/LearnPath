...
function LearningPath() {
  ...
  return (
    <div className="learning-path-container">
      ...
      <p className="total-days">Estimated Duration: {path.totalDaysEstimated} days</p>

      <div className="topics-list">
        {path.topics && path.topics.map((topic, idx) => (
          <div key={idx} className={`topic-card priority-${topic.priority?.toLowerCase()}`}>
            <div className="topic-header">
              <h4>{topic.topicName}</h4>
              <span className="priority-badge">{topic.priority}</span>
            </div>
            <p className="topic-days">Estimated: {topic.estimatedDays} days</p>
            {topic.prerequisites && topic.prerequisites.length > 0 && (
              <p className="prerequisites">Prerequisites: {topic.prerequisites.join(', ')}</p>
            )}
            <div className="topic-status">
              <select 
                onChange={(e) => updateTopicStatus(topic.topicName, e.target.value)}
